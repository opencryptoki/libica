...
static void ctr_inc_block(unsigned char *iv, unsigned int block_size,
			  ...
			  unsigned long ctrlist_length)
{
	...
	__memcpy_r_allign((void *)&tmp_ctr, sizeof(tmp_ctr),
	...
	for (dest = ctrlist;
		...
		__memcpy_r_allign(dest, block_size, (void *)&tmp_ctr,
				  sizeof(tmp_ctr), ctr_byte_width);
		__inc(&tmp_ctr);
	}

	// update iv for chaining
	__memcpy_r_allign(iv, block_size, (void *)&tmp_ctr,
			  sizeof(tmp_ctr), ctr_byte_width);
}

static void ctr_inc_single(unsigned char *iv, unsigned int block_size,
			   unsigned int ctr_width)
{
	...
	// init counter with iv
	__memcpy_r_allign((void *)&tmp_ctr, sizeof(tmp_ctr), iv, block_size, block_size);

	__inc(&tmp_ctr);

	// update iv for chaining
	__memcpy_r_allign(iv, block_size, (void *)&tmp_ctr, sizeof(tmp_ctr), ctr_byte_width);
}
