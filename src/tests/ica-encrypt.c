 ...
 * 
 * ica-encrypt.c - DES-CBC encrypt a file using hardware
 *
 * compile:
 * $ gcc -o ica-encrypt -g -O0 ica-encrypt.c -lica
 * run:
 * $ ./ica-encrypt <infile> <outfile>
 */
...
int main(int argc, char **argv)
{
	...
	rc = icaDesEncrypt( pAdapterHandle,
	...
	printf("%d passed icaDesEncrypt %d bytes\n", __LINE__, bytes_read);
	
	if(rc) {	
		perror("icaDesEncrypt");
		printf("%s %d icaDesEncrypt failed with rc: %d\n", __FILE__, __LINE__, rc);
		goto adapter_close;
	}
	...
	// CBC means iv[i]=C[i-1]
	memcpy(pIV, outBuf + outSize - sizeof(pIV), sizeof(pIV));
	bytes_read = read(infiledes, buf, READ_SIZE);
	while(bytes_read == READ_SIZE) {
		...
		if(rc) {
			perror("icaDesEncrypt");	
			printf("%d icaDesEncrypt failed with rc: %d\n", __LINE__, rc);
			goto adapter_close;
		}
